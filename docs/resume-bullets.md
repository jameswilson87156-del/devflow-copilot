...
- 编写 20 个 JUnit 5 / MockMvc 集成测试，并配置 Docker Compose 与 GitHub Actions 完成后端测试和前端构建验证。

## 30 秒面试介绍

我做了一个 Agentic Coding Workflow 控制台，它不是聊天套壳，而是把项目上下文、Prompt 模板、模型生成、Generation Trace、Agent Run Trace、Knowledge 引用和人工确认组织成固定流程。后端使用 Spring Boot、MyBatis-Plus 和 Flyway，支持 H2 与 MySQL；AI 层支持 OpenAI-compatible Provider 和本地规则降级；每次生成都会记录模型、Token、延迟、模板版本、工具调用和人工确认状态，并通过状态机限制保存和确认操作。项目目前有 20 个后端测试，以及 Docker Compose 和 GitHub Actions。
