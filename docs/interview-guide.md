...
DevFlow Copilot 是一个面向 Java 开发者的 Agentic Coding Workflow 控制台。它不是聊天机器人，而是把项目上下文、需求拆解、日志分析、代码修改计划、修复 Prompt、Generation Trace、Agent Run Trace、Knowledge Base 引用和生成历史组织成可追踪的 Human-in-the-loop 流程。当前 MVP 默认使用本地规则生成结构化 Markdown 输出，也保留 OpenAI-compatible Provider 代码路径。

## 1 分钟项目介绍

这个项目解决的是 AI Coding 使用过程中的混乱问题：需求拆不清、Prompt 不稳定、知识上下文散落、AI 输出和工具调用难追踪。DevFlow Copilot 通过 Dashboard、Workbench、Agent Run Trace、Knowledge Base、Log Analyzer、Prompt Studio 和 Generation History，把一次 AI Coding 行为变成固定流程：选择项目上下文，选择生成类型，可选检索知识库，生成结构化 Artifact，保存历史并人工确认。后端使用 Spring Boot 3，前端使用 Vue3 + TypeScript + Element Plus，默认用 local-rule 保证无 Key 稳定演示。
...
数据模型包括 `project_context`、`prompt_template`、`generation_record`、`generation_trace`、`agent_run`、`agent_step`、`tool_call_record`、`human_review`、`knowledge_document`、`knowledge_chunk`、`generation_knowledge_reference`、`log_analysis` 和 `ai_task`。其中 `generation_record` 是追踪 AI 输出的核心表，`generation_trace` 记录 Prompt 和 Provider 元数据，Agent 相关表记录步骤、工具调用和人工确认。
...
- 可解释 Agent Workflow：记录步骤、工具调用和 Review 状态，但不包装成完整多 Agent Runtime。
- 轻量 RAG 边界：先做文档切片、关键词检索和引用返回，不把它说成向量数据库。
...
当前已经保留 `AiGenerateService` 与 Provider Router，并实现 OpenAI-compatible 代码路径：

1. 通过环境变量配置 `DEVFLOW_AI_PROVIDER=openai-compatible`、`DEVFLOW_AI_BASE_URL`、`DEVFLOW_AI_API_KEY` 和 `DEVFLOW_AI_MODEL`。
2. 从 `prompt_template` 读取模板并渲染变量。
3. 可选检索 Knowledge Base chunks 并注入 Prompt 上下文。
4. 调用兼容 `/v1/chat/completions` 的服务获取 Markdown 输出。
5. 保存到 `generation_record`、`generation_trace` 和 Agent Run Trace，状态仍然是 `READY_FOR_REVIEW`。
6. 保持人工确认流程不变。
...
- Generation Trace、Agent Run Trace 和 Tool Call 记录
- Knowledge Base chunk 引用
...
### Q8：Agent Run Trace 是不是完整多 Agent？

A：不是。当前记录的是一次生成任务的 workflow：任务接收、Prompt 渲染、Knowledge 检索、Provider 调用和人工确认。它用于解释和审计，不做自动多 Agent 协作调度。
