...
4. 保存每次生成的输入、输出、Provider、模型、Token、延迟、模板版本和失败原因，并写入 Generation Trace。
5. 记录 Agent Run、Agent Step、Tool Call 和 Human Review，要求生成记录经过保存和人工确认后进入终态。
6. 管理轻量 Knowledge Base，支持文档切片、关键词检索和生成引用返回。
...
Workbench 请求
  ...
  → 校验并渲染 {{variables}}
  → 创建 GENERATING 记录
  → 可选检索 Knowledge Base chunks
  → 创建 Agent Run / Step / Tool Call
  → Provider Router 选择 local-rule / openai-compatible
  → 保存模型、Token、延迟与输出
  → 写入 Generation Trace 与 Human Review(PENDING)
...
当前没有 SSE 流式输出、登录权限、向量数据库、复杂多 Agent 调度和自动代码执行。Knowledge Base 已实现轻量关键词检索和引用返回；后续若增加长耗时模型调用，可将生成任务异步化并通过 SSE 推送状态。
