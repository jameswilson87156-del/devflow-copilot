...
项目默认使用 `local-rule` Provider，可在没有 API Key 的情况下完整演示；也提供 OpenAI-compatible LLM Provider 适配，配置兼容 `/v1/chat/completions` 的服务地址和 API Key 后可切换到真实模型调用。
...
| AI 工程化 | OpenAI-compatible LLM Provider、local-rule 降级模式、Prompt 模板变量渲染、生成状态机 |
| 工程与交付 | Docker Compose、Nginx、GitHub Actions |
...
- 日志分析：面向 Java / Spring Boot 日志做规则化诊断和修复建议。
- Prompt 模板：管理模板类型、变量、版本、启用状态和默认模板。
- 生成历史：记录 provider、model、token、latency、模板名称、模板版本、失败原因和状态。
...
Compose 会启动 `mysql`、`backend`、`frontend` 三个服务。前端地址为 `http://localhost:5173`，后端地址为 `http://localhost:8080`，MySQL 暴露在 `localhost:3306`。
...
关键覆盖点包括：Prompt 变量渲染、缺少变量校验、Provider 降级、生成记录持久化、状态机、Controller 参数校验、Mapper CRUD 和日志诊断规则。
...
API Key 只通过环境变量注入，不应写入代码或提交到仓库。真实 Provider 调用失败时，系统会按配置降级到 `local-rule`，并在生成记录的 `errorMessage` 中保留原因。
...
- 能说明 Provider 设计：默认 `local-rule` 保障演示闭环，OpenAI-compatible 适配真实模型，失败时可降级并记录错误原因。
- 能说明数据可信度：每次生成记录 provider、model、token、latency、模板版本和状态，方便追踪和复盘。
- 能说明工程化能力：MyBatis-Plus、Flyway、H2/MySQL Profile、DTO 校验、全局异常处理、JUnit 测试、Docker Compose 和 GitHub Actions 都有实际落地。
